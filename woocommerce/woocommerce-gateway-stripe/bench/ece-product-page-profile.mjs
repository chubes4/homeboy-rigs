...
export function setting(name, defaultValue = '') {
  return homeboySetting(name, defaultValue);
}
...
function webperfProfileOptions(profile, metadata, { throttleProfile = null, hintStrategy = 'none' } = {}) {
  ...
  return {
    ...
    stripePublishableKeyEnvName: null,
    stripeSecretKeyEnvName: null,
    runtimePreview: null,
    recipeRunArgs: [],
    browserProbeArgs: [
...
export function buildEceProfileOptions(profile = eceBrowserProfile()) {
  ...
  return {
    ...
    stripePublishableKey: null,
    stripeSecretKey: null,
    stripePublishableKeyEnvName: profile === REAL_WALLET_PROFILE ? 'STRIPE_PUBLISHABLE_KEY' : null,
    stripeSecretKeyEnvName: profile === REAL_WALLET_PROFILE ? 'STRIPE_SECRET_KEY' : null,
