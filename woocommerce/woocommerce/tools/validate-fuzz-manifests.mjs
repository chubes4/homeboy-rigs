...
assert.equal(dbApiPerformanceFuzzerGapReport.readiness?.level, 'declared', 'DB/API gap report workload readiness must stay declared until upstream artifact-postprocess exists');
assert.equal(dbApiPerformanceFuzzerGapReport.readiness?.execution, 'blocked_artifact_aggregation', 'DB/API gap report workload must document blocked artifact aggregation execution');
...
assert.match(dbApiPerformanceFuzzerGapReport.boundary_note || '', /data-only local policy/, 'DB/API gap report must document the data-only postprocess boundary');
...
assert.equal(dbApiFuzzCampaign.postprocess?.runner_support_status, 'blocked', 'DB/API fuzz campaign postprocess must be blocked until upstream artifact-postprocess exists');
assert.match(dbApiFuzzCampaign.postprocess?.boundary_note || '', /artifact-postprocess runner primitive/, 'DB/API fuzz campaign postprocess must document the upstream artifact-postprocess boundary');
...
assert.equal(dbApiFuzzCampaign.operator_commands?.status, 'blocked_pending_upstream_artifact_postprocess', 'DB/API fuzz campaign operator commands must remain blocked on upstream artifact-postprocess');
...
assert.ok(dbApiFuzzCampaign.operator_commands.commands.every((command) => !command.includes('--workload coverage-gap-report')), 'DB/API fuzz campaign must not offer blocked coverage gap postprocess as an executable command');
assert.ok(dbApiFuzzCampaign.operator_commands.commands.every((command) => !command.includes('--workload performance-hotspots-artifact-summary')), 'DB/API fuzz campaign must not offer blocked hotspot postprocess as an executable command');
...
assert.equal(dbApiHotspotArtifactIo.readiness?.level, 'declared', 'DB/API hotspot artifact IO must stay declared until upstream artifact-postprocess exists');
...
assert.equal(coverageGapReportWorkload.metadata?.generic_primitive?.status, 'blocked', 'coverage-gap-report workload must mark artifact-postprocess as blocked');
assert.equal(performanceHotspotsWorkload.metadata?.generic_primitive?.status, 'blocked', 'performance-hotspots workload must mark artifact-postprocess as blocked');
