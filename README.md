...
`studio-site-editor-preload-classic-themes` measures the route-aware preload candidate from WordPress/wordpress-develop#11766 across a block-theme and classic-theme matrix so the `wp_is_block_theme()` guard decision is legible before WordPress 7.1 Beta 1 (reviewer context: t-hamano review 4680372983 on the core PR). It runs baseline (unpatched) and candidate (patched) bot paths over three scenarios in one run: `block` (default block theme, e.g. Twenty Twenty-Five), `classic` (Twenty Twenty-One, editor opens on the patterns route), and `classic-btp` (Twenty Twenty-One plus an mu-plugin calling `add_theme_support( 'block-template-parts' )` so the editor opens in template-parts mode). The candidate is installed verbatim from the PR's full PHP block (template slug resolution, `get_block_templates` lookup, block walk for template parts and `core/query`, front-page/static-page handling, pages list, taxonomies) under the existing `HOMEBOY_SITE_EDITOR_PRELOAD_CANDIDATE` marker; the harness vendors the block with a comment pointing at the PR and commit SHA. Per scenario it records the full client request waterfall (tagged with scenario + bot path), the server-side preloaded REST path set (captured via a `HOMEBOY_SITE_EDITOR_PRELOAD_CAPTURE` snapshot of the final `$preload_paths`), a hit/waste classification (each preloaded path matched against client fetches with normalized query-arg ordering), and wall-time metrics (measure-ready phase, REST network count, server bootstrap timeline). A single roll-up artifact reports `preloaded`, `consumed`, `wasted`, and candidate-vs-baseline timing deltas per scenario — for classic scenarios the wasted-preload count and timing delta are the headline guard-decision numbers:

```bash
homeboy bench --rig studio-combined \
  --scenario studio-site-editor-preload-classic-themes \
  --iterations 1 \
  --shared-state /tmp/studio-preload-classic-themes
```
